...
# Local one-command merge into main (or your integration branch):
#   chmod +x ./scripts/mergeToDevelopBranch.sh
#   ./scripts/mergeToDevelopBranch.sh
#
# Run from a feature branch (not main). The script will:
#   update main → merge main into your branch → merge your branch into main
#
...
warn() {
  printf '[merge-to-main] WARN: %s\n' "$*" >&2
}

die() {
  printf '[merge-to-main] ERROR: %s\n' "$*" >&2
  exit 1
}
...
usage() {
  cat <<'USAGE'
Usage: ./scripts/mergeToDevelopBranch.sh

First-time setup:
  chmod +x ./scripts/mergeToDevelopBranch.sh

What it does:
  Safely merge your current feature branch into local main (the project's integration branch).
...
Flow:
  ...
  5. Ask whether to push to origin main (default: no)
...
prompt_continue() {
  ...
  printf '  4. Ask whether to push to %s %s (default: no)\n' "$GIT_REMOTE" "$TARGET_BRANCH" >&2
  printf '\nContinue? [Y/n]: ' >&2
  read -r reply
...
prompt_push_develop() {
  ...
  if ! is_interactive; then
    log "non-interactive mode; skipped push to ${GIT_REMOTE}/${TARGET_BRANCH}"
    return 0
  fi

  printf 'Push to %s %s? [y/N]: ' "$GIT_REMOTE" "$TARGET_BRANCH" >&2
  read -r reply
  if [[ "$reply" =~ ^[Yy]$ ]]; then
    ...
    log "push skipped. Local ${TARGET_BRANCH} contains the merge from ${ORIGINAL_BRANCH}"
    log "to push later: git push ${GIT_REMOTE} ${TARGET_BRANCH}"
  fi
}
...
main() {
  ...
  log "stay on ${TARGET_BRANCH} to integrate further, or: git checkout ${ORIGINAL_BRANCH}"
}
