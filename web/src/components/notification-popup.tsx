 ...
 * It performs NO sync work. Mount it once in the authenticated layout so a
 * single shared instance is visible from every page.
 *
...
export function NotificationPopUp() {
  ...
  const title = isRunning
    ? t('notification.syncingPercent', { percent: Math.round(progress) })
    : isSuccess
    ...
    >
      ...
      >
            ...
            <div className='min-w-0 overflow-hidden'>
              {itemsCount > 0 && (
                <p className='text-[11px] tabular-nums text-muted-foreground'>
                  {t('notification.items', { count: itemsCount })}
                </p>
              )}
              {resource && (
        ...
        <Button
          ...
          aria-label={
            showDetails ? t('notification.hideDetailsAria') : t('notification.showDetailsAria')
          }
          title={
            showDetails ? t('notification.hideDetailsAria') : t('notification.showDetailsAria')
          }
