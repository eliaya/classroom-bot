      ...
      <SheetContent className='flex flex-col'>
        ...
        <SheetFooter className='gap-2'>
          <Button
            variant='destructive'
            onClick={handleReset}
            aria-label={t('config.resetAllAria')}
          >
            {t('config.resetAll')}
          </Button>
        </SheetFooter>
...
function RadioGroupItem({
  ...
  return (
    <Item
      ...
      aria-label={`${t('config.selectPrefix')} ${item.label.toLowerCase()}`}
  ...
  return (
    <div>
      <SectionTitle
        ...
        resetAriaLabel={t('config.themeResetAria')}
      />
      <Radio
        value={theme}
      ...
      >
        {[
          {
            value: 'system',
            label: t('config.labels.system'),
            icon: IconThemeSystem,
          },
          {
            value: 'light',
            label: t('config.labels.light'),
            icon: IconThemeLight,
          },
          {
            value: 'dark',
            label: t('config.labels.dark'),
            icon: IconThemeDark,
          },
        ].map((item) => (
    ...
    <div className='max-md:hidden'>
      ...
      >
        {[
          {
            value: 'inset',
            label: t('config.labels.inset'),
            icon: IconSidebarInset,
          },
          {
            value: 'floating',
            label: t('config.labels.floating'),
            icon: IconSidebarFloating,
          },
          {
            value: 'sidebar',
            label: t('config.labels.sidebar'),
    ...
    <div className='max-md:hidden'>
      ...
      >
        {[
          {
            value: 'default',
            label: t('config.labels.default'),
            icon: IconLayoutDefault,
          },
          {
            value: 'icon',
            label: t('config.labels.compact'),
            icon: IconLayoutCompact,
          },
          {
            value: 'offcanvas',
            label: t('config.labels.fullLayout'),
            icon: IconLayoutFull,
          },
        ].map((item) => (
          <RadioGroupItem key={item.value} item={item} />
  ...
  return (
    <div>
      ...
      <Radio
        ...
        className='grid w-full max-w-md grid-cols-3 gap-4'
        aria-label={t('config.direction')}
        aria-describedby='direction-description'
      >
        {[
          {
            value: 'ltr',
            label: t('config.labels.ltr'),
